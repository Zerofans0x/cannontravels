...
export default function DashboardSidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const navSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: "Transaction", href: "/dashboard/transaction", icon: "bi:journal" },
        { name: "Flight Schedule", href: "/dashboard/flight-schedule", icon: "lucide:calendar" },
      ],
    },
    {
      title: "OPERATIONS",
      items: [
        { name: "Track & Trace", href: "/dashboard/track", icon: "lucide:radar" },
  ...
  return (
    <>
      ...
      >
        <div className="w-full h-full max-h-[96vh] bg-[#1a0505] text-white rounded-[20px] p-4 sm:p-5 flex flex-col justify-between shadow-2xl overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-white/10 border border-red-950/40">
          <div>
            {/* Desktop Top Logo */}
            ...
            <div className="space-y-3.5 sm:space-y-4">
              ...
              {/* Logout Button added right after Navigation sections for both Mobile & Laptop */}
              <div className="pt-2">
                <button
                  type="button"
                  onClick={logout}
                  className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-[12px] text-[13px] font-medium text-rose-400 hover:bg-red-950/40 hover:text-rose-300 transition-all cursor-pointer"
                >
                  <Icon icon="lucide:log-out" className="w-4 h-4 flex-shrink-0 text-rose-400" />
                  <span>Sign Out</span>
                </button>
              </div>
            </div>
