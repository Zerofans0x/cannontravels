  ...
  useEffect(() => {
    const verifyTransaction = async () => {
      ...
      try {
        ...
        // Safely map backend data to state
        if (response.data?.bookingId) setVerifiedBookingId(response.data.bookingId);
        if (response.data?.trackingCode) setTrackingCode(response.data.trackingCode);
        if (response.data?.isDelegated) setIsDelegated(response.data.isDelegated);
    ...
    <div className="w-full max-w-[480px] bg-white border border-slate-200 rounded-[32px] p-8 text-center shadow-xl">
      ...
      {status === "verifying" && (
        <div className="flex flex-col items-center space-y-4 py-8">
          ...
          <h2 className="text-xl font-black text-slate-900 tracking-tight">Verifying Payment Settlement</h2>
          <p className="text-xs text-slate-500 leading-relaxed max-w-xs">
            Confirming gateway clearance, securing the PNR seat, and activating airspace telemetry...
          </p>
        </div>
      )}
      ...
      {status === "success" && (
        <div className="flex flex-col items-center space-y-5 py-4 animate-in fade-in duration-300">
          <div className="w-16 h-16 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-full flex items-center justify-center shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 leading-relaxed mt-1">
              {isDelegated 
                ? "The flight ticket has been successfully paid for and issued to the passenger's email. Telemetry is now active." 
                : "Your flight ticket has been issued successfully. You can now view your Boarding Pass or return to your terminal dashboard."}
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 w-full pt-2">
            {/* 🟢 Delegated View: Show only the Live Tracking radar link */}
            {isDelegated && trackingCode ? (
              <button
                onClick={() => router.push(`/track/${trackingCode}`)}
                className="w-full py-3.5 bg-[#DC2626] hover:bg-[#B91C1C] text-white font-bold text-sm rounded-xl cursor-pointer shadow-md shadow-red-600/20 transition-all flex items-center justify-center gap-2"
              >
                <Icon icon="lucide:radar" className="w-5 h-5" />
                <span>Proceed to Live Flight Tracking</span>
              </button>
            ) : (
              /* Normal User View: Dashboard & Ticket links */
              <>
                {verifiedBookingId ? (
                  <button
                    onClick={() => router.push(`/dashboard/bookings/${verifiedBookingId}`)}
